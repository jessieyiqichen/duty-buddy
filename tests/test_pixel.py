...
from dutyboard.pixel import AISLE_W, DESK_GAP, DESK_W, WALL_H, build_scene, pile_hit, pose_for, seat_at, skin_for

NOW = datetime(2026, 9, 22, 12, 0, tzinfo=timezone.utc)


def _e(icon, title="改简历", age=60):
    return Entry(title, icon, NOW - timedelta(seconds=age), "local_" + title, "code")
...
def test_scene_layout_wraps_rows_caps_seats_and_desks():
    projects = tuple(ProjectView(f"p{i}", 0, (_e("🟢"), _e("🟡"), _e("·"), _e("·"))) for i in range(8))
    scene = build_scene(projects, stale=5, now=NOW, cols=3, max_desks=6)
    assert len(scene.desks) == 6 and scene.hidden_desks == 2
    assert scene.desks[0].y == WALL_H and scene.desks[0].y == scene.desks[2].y and scene.desks[3].y > scene.desks[0].y
    assert scene.desks[1].x == AISLE_W + DESK_W + DESK_GAP
    assert len(scene.desks[0].seats) == 3
...
def test_bubbles_stack_and_stay_inside():
    ...
    projects = (ProjectView("p", 3, (_e("🟡", title="改简历"), _e("🟡", title="负一屏专项需求"), _e("🟠", title="集成"))),)
    scene = build_scene(projects, stale=0, now=NOW)
    bubbles = layout_bubbles(scene.desks[0], scene.width)
    assert [b.text for b in bubbles] == ["改简历", "负一屏专项需求", "? 集成"]
